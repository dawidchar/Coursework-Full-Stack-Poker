...
let formatedPlayerCards
export function CalculateRank(Cards, handObject) {
    var Ranking = new Hand(convertFormat(Cards)); //convert the cards that are currently instances to just 2 Letters The suit and rank
    console.log("Ranking", Ranking)
    handObject.PokerRank = Ranking.message;
    handObject.PokerRankValue = Ranking.rank;
    handObject.formatedPlayerCards = formatedPlayerCards;
}


function convertFormat(cards) {
    var outputArray = [];
    for (var i = 0; i < 7; i++) { // For all 7 cards get the suit and rank and combine their character together 
        var cardRank = cards[i].rank
        var cardSuit = cards[i].suit;
        if (cardRank == "10"){cardRank = "T"}; // Replace any 10s with a T
        outputArray.push(cardSuit.concat(cardRank)) // Join the Suit and Rank
    }
    formatedPlayerCards = outputArray[0][1] + "(" + outputArray[0][0]+")" +  outputArray[1][1] + "(" + outputArray[1][0]+")" //For displaying to players
...
function rankHandInt(hand) {
    ...
    for (i = 0; i < hand.cards.length; i += 1) {
        handRanks[i] = hand.cards[i].substr(1, 1);
        handSuits[i] = hand.cards[i].substr(0, 1);
    }

    ranks = handRanks.sort().toString().replace(/\W/g, "");
    suits = handSuits.sort().toString().replace(/\W/g, "");
    cards = hand.cards.toString();
    console.log({"rankHandInt":[{"ranks":ranks},{"suits":suits}]})
    //Four of a kind
    if (rank === 0) {
    ...
    console.log("internal Rank", rank);
    return {
        rank: rank,
        message: message
    }
